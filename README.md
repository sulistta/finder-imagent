> Current runtime: this repository is being migrated to **Localhost Image Finder**, a local-only Node + React app for uploading XLSX workbooks, mapping product columns, running Google/Gemini model agents, and downloading a filled XLSX. The VS Code/Claude extension README below is legacy project history and is not the active runtime path.
>
> Quick start:
>
> ```sh
...
> ```
>
> Open `http://127.0.0.1:5174`. Configure `GOOGLE_API_KEYS` or `GOOGLE_API_KEY_1`, plus `GEMINI_QUERY_MODEL`, `GEMINI_RANKING_MODEL`, `GEMINI_VISUAL_MODEL`, and `GEMINI_METADATA_MODEL` in `.env`. Local jobs are stored under `.image-finder-data/`; writes are `fill-empty-only`.
